...
class Trainer(object):
    ...
    def mapping_f2loss_step(self, stats):
        """
        Fooling discriminator training step.
        """
        bs = self.params.batch_size
        data_ids = torch.LongTensor(bs).random_(int(0.1*len(self.src_dico)))
        if self.params.cuda:
            data_ids = data_ids.cuda()

        # get node embeddings
        src_emb = self.src_emb(Variable(data_ids))
        tgt_emb = self.tgt_emb(Variable(data_ids))
        src_emb = self.mapping(Variable(src_emb.data))
        tgt_emb = Variable(tgt_emb.data)
        # loss
        loss = F.pairwise_distance(src_emb, tgt_emb, p=2)
        loss = torch.sum(loss)
        print(loss)
        loss = 0.001*self.params.dis_lambda * loss
    ...
    #     """
    #     node2id1 = self.src_dico.node2id
    #     node2id2 = self.tgt_dico.node2id

    #     # identical character strings
    #     if dico_train == "identical_char":
    #         self.dico = load_identical_char_dico(node2id1, node2id2)
    #     # use one of the provided dictionary
    #     elif dico_train == "default":
    #         filename = '%s-%s.0-5000.txt' % (self.params.src_graph, self.params.tgt_graph)
    ...
    #     else:
    #         self.dico = load_dictionary(dico_train, node2id1, node2id2)
    ...
    def export(self):
        ...
        params.src_dico, src_emb = load_embeddings(params, source=True, full_node=True)
        params.tgt_dico, tgt_emb = load_embeddings(params, source=False, full_node=True)
